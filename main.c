...
void main()
{
	...
	*/
	...
	if (!search_value(tree, 11)) insert_value(tree, 11); // check existence before insertion
	...
	if (search_value(tree, 10)) delete_value(tree, 10); // check existence before insertion
	//postorder_traverse(tree);
	preorder_traverse(tree);
	printf("\n");
	printf("Depth of tree is: %d, Number of nodes is: %d, Number of leaves is: %d \n", get_depth(tree), nodes_count(tree), leaves_count(tree));
